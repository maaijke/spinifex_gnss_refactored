...
def _parse_sp3_header_flexible(lines: list[str]) -> tuple[SP3Header, int]:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"Could not parse date/time from first line: {first_line}"
            ) from e2

    # start_time = Time(datetime(year, month, day, hour, minute, int(second)) +
    #                 timedelta(seconds=second - int(second)))
    start_time = (
        Time(f"{year}-{month}-{day}T{hour}:{minute}:{second}", scale="tai")
        + GPS_TO_TAI_SECONDS * u.s
    )
...
def get_satellite_position(
    sp3_data: SP3Data, satellite_id: str, times: Time
) -> EarthLocation:
    """
    ...
    # SP3 times in MJD
    sp3_times_mjd = sp3_data.times.utc.mjd
    target_times_mjd = times.utc.mjd
    ...
    return EarthLocation(x=x_interp * u.m, y=y_interp * u.m, z=z_interp * u.m)
...
def concatenate_sp3_files(sp3_files: list[Path]) -> SP3Data:
    ...
    # Concatenate times
    all_times = Time(
        np.concatenate([data.times.mjd for data in parsed_files]),
        format="mjd",
        scale=parsed_files[0].times.scale,
    )
