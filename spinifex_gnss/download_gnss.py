...
async def _download_sp3_file_with_fallback(
    date: datetime, base_url: str, datapath: Path
) -> Path:
    """
...
def _build_rinex3_filenames(station: str, year: int, doy: int) -> str:
    """
    ...
    return [
        ...
        f"{station}_U_{year}{doy:03d}0000_01D_30S_MO.crx.gz",
        f"{station}_S_{year}{doy:03d}0000_01D_30S_MO.crx.gz",
    ]
...
def _build_rinex2_filenames(station: str, year: int, doy: int) -> list[str]:
    ...
    filenames = [
        ...
        f"{station_4char}{doy:03d}0.{yy}o.gz".upper(),  # Standard
        f"{station_4char}{doy:03d}0.{yy}d.gz".upper(),  # Daily
...
def check_local_rinex_files(
    ...
    for station in stations:
        ...
        rinex3_patterns = [
            ...
            f"{station}_U_{year}{doy:03d}0000_01D_30S_MO.crx.gz",
            f"{station}_U_{year}{doy:03d}0000_01D_30S_MO.crx",
            f"{station}_U_{year}{doy:03d}0000_01D_30S_MO.rnx.gz",
            f"{station}_U_{year}{doy:03d}0000_01D_30S_MO.rnx",
            f"{station}_S_{year}{doy:03d}0000_01D_30S_MO.crx.gz",
            f"{station}_S_{year}{doy:03d}0000_01D_30S_MO.crx",
            f"{station}_S_{year}{doy:03d}0000_01D_30S_MO.rnx.gz",
            f"{station}_S_{year}{doy:03d}0000_01D_30S_MO.rnx",
        ]
        ...
        if found_path is None:
            ...
            rinex2_patterns = [
                ...
                f"{station_4char}{doy:03d}0.{yy}o.gz".upper(),
                f"{station_4char}{doy:03d}0.{yy}d.gz".upper(),
                f"{station_4char}{doy:03d}0.{yy}O.gz",
...
async def download_rinex_coro(
    ...
    if urls:
        ...
        coros = [
            download_or_copy_url(url, output_directory=datapath) for url, _ in urls
        ]
